...
const invokeAgMethod = client => {
    ...
    glue.agm.invoke("SetParty", { party: client });
};
...
const openWindow = (windowName, myWin, direction) => {
    ...
    const options = {
        ...
        context: { parentWindowId: myWin.id }
    };
...
const openTabWindow = (party, direction) => {
    ...
    const isTherePortfolioTab = glue.windows
        .list()
        .find(wnd => wnd.name.includes("PortfolioTabs"));
    const options = {
        mode: "tab",
        tabGroupId: "PortfolioTabs",
        relativeTo: isTherePortfolioTab ? undefined : glue.windows.my().id,
        relativeDirection: isTherePortfolioTab ? undefined : direction,
        context: {
            party: party,
